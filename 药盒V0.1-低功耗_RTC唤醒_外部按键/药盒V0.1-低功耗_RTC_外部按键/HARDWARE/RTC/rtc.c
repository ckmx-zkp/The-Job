...
{	
		...
		NVIC_InitStructure.NVIC_IRQChannelPreemptionPriority = 0x01;	//先占优先级1位,从优先级3位
		NVIC_InitStructure.NVIC_IRQChannelSubPriority = 0x04;	//先占优先级0位,从优先级4位
...
u8 RTC_Init(void)
{
	...
	{	 			
		...
		RTC_WaitForLastTask();	//等待最近一次对RTC寄存器的写操作完成
		RTC_Set(2015,3,27,9,48,55);  //设置时间	
...
//			printf("通过RTC进入正常模式!");
//			EXTI_ClearITPendingBit(RTCAlarm_IRQn);  	
//  } 				  								 
